        ...
        for (mention <- orderedBySentence.getOrElse(i, Seq.empty[Mention])) {
          ...
          if (existsDeterminerCause(mention)) {
            ...
            if (prevSentenceCausal.nonEmpty) {
              ...
              // Make a new mention using the previous effect as the current cause
              // Note: Likely overly simplistic, this is a first pass
              val currEffects = mention.asInstanceOf[EventMention].arguments("effect")
              val newArgs = scala.collection.mutable.Map[String, Seq[Mention]]()
              newArgs.put("cause", prevEffects)
